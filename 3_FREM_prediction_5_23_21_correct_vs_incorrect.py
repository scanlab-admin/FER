		...
		for threshold in thresholds: 
			for L2_contrast in L2_contrasts:
				for i in range(1,4+1,1):
					...
					file = pathlib.Path(path2data+'/roi_transforms/' + L1_contrast + '/' + threshold + '/' + L2_contrast + '/_' +ROI_number+'accuracy_correct_v_incorrect.txt')
					...
					if file.exists ():
						...
						try: 
							...
							coef_img_correct = decoder.coef_img_['correct']

							#uncomment if saving the svc weights.  Probably dont need them, since all we are interested in is the accuracy. 
							#decoder.coef_img_['correct'].to_filename(path2data+'%s_FREM_correct_svc_weights.nii.gz'%subject)
							...
							result_file=path2data+'/roi_transforms/' + L1_contrast + '/' + threshold + '/' + L2_contrast + '/_' +ROI_number+'accuracy_correct_v_incorrect.txt'

							text_file = open(result_file, "w")
							text_file.write("{:.3f}".format(null_cv_scores.mean()*100))
							text_file.write(", %s" % accuracy)
							text_file.close()
		
						except:
					
							print("FREM not run on" +subject + ' ' + ROI_number + ' '+ L1_contrast + ' '+ L2_contrast + " - mask probably empty")