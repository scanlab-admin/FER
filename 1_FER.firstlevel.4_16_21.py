...
Created:		05-17-2021	# based on Domain pipeline script (J.Richey)
Code Revised:	5-29-2021	# update to incorporate ASD subjects (Prepended "EMOXXX")
"""
...
def get_events(subject_id):
	...
	event_1		= path2events+'/%(MYSUBJECT)s/run1emrec2sec_2SEC_duration/all_cor.run001.txt' %{"MYSUBJECT":(subject_id)}
	event_2		= path2events+'/%(MYSUBJECT)s/run1emrec2sec_2SEC_duration/all_inc.run001.txt'%{"MYSUBJECT":(subject_id)}
...
l1pipeline = pe.Workflow(name="l1pipeline_HDR_2_sec_w_both_derivs_2021")
...
contrastestimate = pe.Node(
	...
	#contrast3 = ('contrast2',	'T', 			['all_inc','all_cor'],			[1,-1]	)
	#contrast4 = ('contrast2',	'T', 			['all_inc','all_cor'],			[-1,1]	)
